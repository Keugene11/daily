...
function buildSystemPrompt(request) {
    ...
    if (days && days > 1) {
        ...
        extras.push(`- MULTI-DAY VACATION MODE (${days} days): Plan a cohesive ${days}-day trip.\n` +
...
- **REQUIRED — BULLET SUMMARY**: End each time section (Morning, Afternoon, Evening, Night) with a bullet list summarizing that section's stops. Do NOT create a separate heading or subsection — just put the bullets at the bottom of the same section, right before the next ## heading. One bullet per stop, formatted like:
  ...
  Every time section must end with these bullets.
...
async function* streamPlanGeneration(request) {
    ...
    if (request.mood)
        userParts.push(`How I'm feeling: "${request.mood}"`);
    if (request.energyLevel)
        userParts.push(`Energy level: ${request.energyLevel}`);
    if (request.dietary && request.dietary.length > 0)
        userParts.push(`Dietary needs: ${request.dietary.join(', ')}`);
    if (request.accessible)
        userParts.push(`I need wheelchair accessible venues`);
    if (request.dateNight)
        userParts.push(`This is a date night — make it romantic`);
    if (request.recurring)
        userParts.push(`Plan my next 4 Saturdays with variety`);
