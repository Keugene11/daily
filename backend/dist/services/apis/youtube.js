...
async function searchYouTubeVideo(query) {
    ...
    for (const r of results) {
        if (await isEmbeddable(r.videoId))
            return r;
    }
...
async function isEmbeddable(videoId) {
    try {
        const res = await fetchWithTimeout(`https://www.youtube.com/embed/${videoId}`, { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36' }, 3000);
        if (!res.ok)
            return false;
        const html = await res.text();
