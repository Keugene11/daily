...
async function fetchOpenMeteo(city) {
    ...
    try {
        ...
        if (gustKmh && gustKmh > windKmh)
            windDesc += `, gusts up to ${gustKmh} km/h`;
        ...
        const maxRainChance = Math.max(...hourlyRain.map((v) => v || 0));
        if (maxRainChance > 30)
            precipDesc += ` | ${maxRainChance}% chance of rain today`;
        // Hourly forecast
        const forecastParts = [];
