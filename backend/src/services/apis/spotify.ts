...
async function fetchDeezerPreview(artist: string, title: string): Promise<string> {
  try {
    const query = encodeURIComponent(`${artist} ${title}`);
...
const VIBE_POOLS: Record<string, { tracks: Track[]; names: string[]; descriptions: string[] }> = {
  chill: {
    ...
    tracks: [
      ...
      track('Holocene', 'Bon Iver'),
      track('First Day of My Life', 'Bright Eyes'),
      track('Such Great Heights', 'Iron & Wine'),
      track('Sea of Love', 'Cat Power'),
      track('Moon River', 'Frank Ocean'),
      track('Myth', 'Beach House'),
      track('Space Song', 'Beach House'),
      track('Flightless Bird American Mouth', 'Iron & Wine'),
      track('Banana Pancakes', 'Jack Johnson'),
      track('Better Together', 'Jack Johnson'),
      track('Lost in the Light', 'Bahamas'),
      track('Electric', 'Alina Baraz'),
      track('Nikes', 'Frank Ocean'),
      track('Agnes', 'Glass Animals'),
      track('Motion Sickness', 'Phoebe Bridgers'),
      track('Liability', 'Lorde'),
      track('The Less I Know the Better', 'Tame Impala'),
      track('Eventually', 'Tame Impala'),
      track('Let It Happen', 'Tame Impala'),
      track('Lemon Glow', 'Beach House'),
  ...
  upbeat: {
    ...
    tracks: [
      ...
      track('Just Like Heaven', 'The Cure'),
      ...
      track('Peach Pit', 'Peach Pit'),
      track('Say It Ain\'t So', 'Weezer'),
    ]
  },
  ...
  adventure: {
    ...
    tracks: [
      track('Runaway', 'Aurora'),
      track('Midnight City', 'M83'),
      track('Intro', 'M83'),
      track('Mykonos', 'Fleet Foxes'),
      track('White Winter Hymnal', 'Fleet Foxes'),
      track('Little Talks', 'Of Monsters and Men'),
      track('Dirty Paws', 'Of Monsters and Men'),
      track('Crystals', 'Of Monsters and Men'),
      track('Youth', 'Glass Animals'),
      track('Gooey', 'Glass Animals'),
      track('Stubborn Love', 'The Lumineers'),
      track('Ophelia', 'The Lumineers'),
      track('Home', 'Edward Sharpe & The Magnetic Zeros'),
      track('On Top of the World', 'Imagine Dragons'),
      track('1901', 'Phoenix'),
      track('Lisztomania', 'Phoenix'),
      track('Budapest', 'George Ezra'),
      track('Shotgun', 'George Ezra'),
      track('Riptide', 'Vance Joy'),
      track('Georgia', 'Vance Joy'),
      track('Innerbloom', 'Rufus Du Sol'),
      track('Solace', 'Rufus Du Sol'),
      track('Carried Away', 'Passion Pit'),
      track('Sleepyhead', 'Passion Pit'),
      track('Take a Walk', 'Passion Pit'),
      track('Something Good Can Work', 'Two Door Cinema Club'),
      track('What You Know', 'Two Door Cinema Club'),
      track('Undercover Martyn', 'Two Door Cinema Club'),
      track('Doses & Mimosas', 'Cherub'),
      track('Trojans', 'Atlas Genius'),
    ]
  },
  ...
  romantic: {
    ...
    tracks: [
      ...
      track('My Funny Valentine', 'Chet Baker'),
      track('Lover', 'Taylor Swift'),
      track('Adorn', 'Miguel'),
      track('Best Part', 'Daniel Caesar'),
      track('Get You', 'Daniel Caesar'),
      track('Electric', 'Khalid'),
      track('Location', 'Khalid'),
      track('Falling', 'Harry Styles'),
      track('Ivy', 'Frank Ocean'),
      track('Thinkin Bout You', 'Frank Ocean'),
      ...
      track('From the Dining Table', 'Harry Styles'),
    ]
  },
  ...
  contemplative: {
    ...
    tracks: [
      ...
      track('An Ending', 'Brian Eno'),
      track('Music for Airports 1/1', 'Brian Eno'),
      track('Intro', 'The xx'),
      track('Angels', 'The xx'),
      track('Crystalised', 'The xx'),
      track('VCR', 'The xx'),
      track('Everything in Its Right Place', 'Radiohead'),
      track('How to Disappear Completely', 'Radiohead'),
      track('Nude', 'Radiohead'),
      track('Teardrop', 'Massive Attack'),
      track('Unfinished Sympathy', 'Massive Attack'),
      track('Glory Box', 'Portishead'),
      track('To Build a Home', 'The Cinematic Orchestra'),
      track('Nuvole Bianche', 'Ludovico Einaudi'),
      track('Experience', 'Ludovico Einaudi'),
      track('On the Nature of Daylight', 'Max Richter'),
      track('Arrival of the Birds', 'The Cinematic Orchestra'),
      track('Outro', 'M83'),
      track('Saturn', 'Sleeping At Last'),
      track('Retrograde', 'James Blake'),
      track('Limit to Your Love', 'James Blake'),
      track('First Breath After Coma', 'Explosions in the Sky'),
      track('Your Hand in Mine', 'Explosions in the Sky'),
      track('Svefn-g-englar', 'Sigur Ros'),
      track('Time', 'Hans Zimmer'),
      track('Mountains', 'Hans Zimmer'),
  ...
  groovy: {
    ...
    tracks: [
      ...
      track('Outstanding', 'The Gap Band'),
      track('Move on Up', 'Curtis Mayfield'),
      track('September', 'Earth Wind & Fire'),
      track('Me and Your Mama', 'Childish Gambino'),
    ]
  },
  ...
  indie: {
    ...
    descriptions: [
      'Underground-ish picks that feel like a personal discovery',
      'Indie tracks that deserve more plays',
      'Songs your friend with good taste would recommend',
    ],
    tracks: [
      track('Obstacles', 'Syd Matters'),
      track('Do You Realize??', 'The Flaming Lips'),
      track('Two Headed Boy', 'Neutral Milk Hotel'),
      track('In the Aeroplane Over the Sea', 'Neutral Milk Hotel'),
      track('Caring Is Creepy', 'The Shins'),
      track('New Slang', 'The Shins'),
      track('Such Great Heights', 'The Postal Service'),
      track('The District Sleeps Alone Tonight', 'The Postal Service'),
      track('Maps', 'Yeah Yeah Yeahs'),
      track('Heads Will Roll', 'Yeah Yeah Yeahs'),
      track('Tessellate', 'alt-J'),
      track('Breezeblocks', 'alt-J'),
      track('Lazy Eye', 'Silversun Pickups'),
      track('Young Folks', 'Peter Bjorn and John'),
      track('Sprawl II', 'Arcade Fire'),
      track('Wake Up', 'Arcade Fire'),
      track('Ready to Start', 'Arcade Fire'),
      track('Sometime Around Midnight', 'The Airborne Toxic Event'),
      track('The Suburbs', 'Arcade Fire'),
      track('No Cars Go', 'Arcade Fire'),
      track('All My Friends', 'LCD Soundsystem'),
      track('Someone Great', 'LCD Soundsystem'),
      track('Dance Yrself Clean', 'LCD Soundsystem'),
      track('Fitzpleasure', 'alt-J'),
      track('The Rat', 'The Walkmen'),
      track('Fireworks', 'Animal Collective'),
      track('My Girls', 'Animal Collective'),
      track('Blue Ridge Mountains', 'Fleet Foxes'),
      track('Dissolve Me', 'alt-J'),
      track('Mykonos', 'Fleet Foxes'),
    ]
  },
  ...
  melancholy: {
    ...
    tracks: [
      track('Skinny Love', 'Bon Iver'),
      track('Holocene', 'Bon Iver'),
      track('Creature Fear', 'Bon Iver'),
      track('The Night We Met', 'Lord Huron'),
      track('Youth', 'Daughter'),
      track('Smother', 'Daughter'),
      track('Motion Sickness', 'Phoebe Bridgers'),
      track('Funeral', 'Phoebe Bridgers'),
      track('I Know the End', 'Phoebe Bridgers'),
      track('Liability', 'Lorde'),
      track('Writer in the Dark', 'Lorde'),
      track('Supercut', 'Lorde'),
      track('Black', 'Pearl Jam'),
      track('Nutshell', 'Alice in Chains'),
      track('Mad World', 'Gary Jules'),
      track('Everybody Hurts', 'R.E.M.'),
      track('Fake Plastic Trees', 'Radiohead'),
      track('No Surprises', 'Radiohead'),
      track('Street Spirit', 'Radiohead'),
      track('Last Goodbye', 'Jeff Buckley'),
      track('Hallelujah', 'Jeff Buckley'),
      track('Between the Bars', 'Elliott Smith'),
      track('Angeles', 'Elliott Smith'),
      track('For Emma', 'Bon Iver'),
      track('Love Will Tear Us Apart', 'Joy Division'),
      track('Atmosphere', 'Joy Division'),
      track('Asleep', 'The Smiths'),
      track('How Soon Is Now', 'The Smiths'),
      track('Pictures of You', 'The Cure'),
      track('Disintegration', 'The Cure'),
    ]
  },
};
...
const CITY_PLAYLISTS: Record<string, PlaylistSuggestion> = {
  'new york': {
    name: 'NYC Soundtrack',
  ...
  'chicago': {
    ...
    tracks: [
      ...
      track('Your Love', 'Frankie Knuckles'),
      track('Can You Feel It', 'Mr. Fingers'),
      track('Pusherman', 'Curtis Mayfield'),
    ]
  },
  'san francisco': {
    name: 'Bay Area Vibes',
  ...
  'london': {
    ...
    tracks: [
      ...
      track('A Town Called Malice', 'The Jam'),
      track('Bonkers', 'Dizzee Rascal'),
      track('West End Girls', 'Pet Shop Boys'),
      track('Parklife', 'Blur'),
      track('Common People', 'Pulp'),
...
function moodToVibes(mood?: string): string[] {
  if (!mood) return [];
  const m = mood.toLowerCase();

  if (/terrible|awful|sad|rough|bad|stressed|anxious|overwhelm/i.test(m)) return ['chill', 'melancholy'];
  if (/tired|exhausted|sleepy|drained|low energy/i.test(m)) return ['chill', 'contemplative'];
  if (/chill|relax|calm|peaceful|easy|lazy/i.test(m)) return ['chill', 'tropical'];
  if (/adventur|explor|discover|wander|curious/i.test(m)) return ['adventure', 'upbeat'];
  if (/excit|hype|wired|pumped|energi|ready/i.test(m)) return ['upbeat', 'nightout'];
  if (/romantic|date|love|intimate|together/i.test(m)) return ['romantic', 'chill'];
  if (/party|dance|club|night out|going out/i.test(m)) return ['nightout', 'groovy'];
  if (/creative|art|inspired|deep|think/i.test(m)) return ['contemplative', 'indie'];
  if (/happy|great|amazing|wonderful|fantastic/i.test(m)) return ['upbeat', 'groovy'];
  if (/funky|groove|soul|retro|vintage/i.test(m)) return ['groovy', 'upbeat'];
  if (/indie|alt|underground|hipster/i.test(m)) return ['indie', 'adventure'];

  return [];
}

// ── Interest → vibe mapping ─────────────────────────────────────────────

function interestToVibes(interests: string[]): string[] {
  const vibes: string[] = [];
  for (const interest of interests) {
    const i = interest.toLowerCase();
    if (i === 'outdoors') vibes.push('adventure', 'upbeat');
    else if (i === 'food') vibes.push('groovy', 'chill');
    else if (i === 'culture') vibes.push('contemplative', 'indie');
    else if (i === 'nightlife') vibes.push('nightout', 'groovy');
    else if (i === 'music') vibes.push('indie', 'groovy');
...
const SONG_REASONS: Record<string, string> = {
  // Chill
  ...
  // Melancholy
  'creature fear|bon iver': 'Bon Iver building from silence to catharsis',
  'smother|daughter': 'Daughter\'s most suffocating and beautiful track',
  'funeral|phoebe bridgers': 'Phoebe Bridgers writing about mortality with grace',
  'i know the end|phoebe bridgers': 'Building from whisper to apocalyptic scream',
  'writer in the dark|lorde': 'Lorde\'s voice cracking with real heartbreak',
  'supercut|lorde': 'Nostalgia as a bittersweet dance song',
  ...
  'fake plastic trees|radiohead': 'Thom Yorke\'s voice breaking under the weight of artifice',
  'no surprises|radiohead': 'Lullaby-gentle despite being desperately sad',
  'hallelujah|jeff buckley': 'Buckley turned Cohen\'s hymn into something transcendent',
  'between the bars|elliott smith': 'Whispering intimacy from a quiet genius',
  'love will tear us apart|joy division': 'Post-punk\'s most devastating love song',
  'pictures of you|the cure': 'Robert Smith at his most wistfully gorgeous',
  'how soon is now|the smiths': 'Morrissey\'s loneliness over tremolo guitar',
  'asleep|the smiths': 'The gentlest, saddest lullaby',
...
function generateReason(t: Track, vibe: string, city?: string): string {
...
export const spotifyService = {
  async getPlaylist(city: string, interests: string[], mood?: string): Promise<ToolResult<PlaylistSuggestion>> {
    ...
    for (const vibe of vibes) {
      const pool = VIBE_POOLS[vibe];
      if (pool) candidateTracks.push(...pool.tracks);
    }
