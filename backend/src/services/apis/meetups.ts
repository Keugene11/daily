...
interface TechEvent {
  ...
  location: string;
  description: string;
  category: 'meetup' | 'hackathon' | 'coworking' | 'workshop' | 'networking';
...
const CITY_MEETUPS: Record<string, TechEvent[]> = {
  'new york': [
...
const DEFAULT_MEETUPS: TechEvent[] = [
...
export const meetupService = {
  async getMeetups(city: string, rightNow?: boolean, localHour?: number): Promise<ToolResult> {
    ...
    const events: MeetupItem[] = available.map(e => {
