...
interface TransitStep {
  instruction: string;
  mode: 'walk' | 'bus' | 'subway' | 'train' | 'tram' | 'ferry';
  line?: string;
  duration: string;
  detail: string;
}
...
const CITY_ROUTES: Record<string, (from: string, to: string) => TransitRouteData> = {
  'new york': (from, to) => ({
    city: 'New York',
    from, to,
    routes: [
...
const defaultRoutes = (city: string, from: string, to: string): TransitRouteData => ({
  city, from, to,
  routes: [
    {
      totalDuration: '20-40 min',
...
function matchCity(city: string, from: string, to: string): TransitRouteData {
  const c = city.toLowerCase().trim();
  for (const [key, fn] of Object.entries(CITY_ROUTES)) {
    if (c.includes(key) || key.includes(c)) return fn(from, to);
  }
  return defaultRoutes(city, from, to);
}

export const transitRouteService = {
  async getTransitRoutes(city: string, from: string, to: string): Promise<ToolResult<TransitRouteData>> {
