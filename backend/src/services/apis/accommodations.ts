...
function getCacheKey(city: string, budget?: string, type?: string): string {
  return `accommodations|${city.toLowerCase().trim()}|${budget || ''}|${type || ''}`;
}

function getFromCache(key: string): Accommodation[] | null {
...
function setCache(key: string, data: Accommodation[]): void {
  if (cache.size > 200) {
...
function mapPlaceToAccommodation(place: any, city: string): Accommodation {
...
async function searchGooglePlaces(
  ...
  try {
    ...
    return places.map((p: any) => mapPlaceToAccommodation(p, city));
  } catch (error: any) {
...
export const accommodationService = {
  async getAccommodations(city: string, budget?: string, type?: string): Promise<ToolResult<Accommodation[]>> {
    ...
    if (type === 'hostel') query = `best hostels in ${city}`;
    else if (type === 'boutique') query = `best boutique hotels in ${city}`;
    else if (type === 'apartment') query = `best apartment hotels serviced apartments in ${city}`;
    if (budget === 'low') query += ' affordable budget';
    if (budget === 'high') query += ' luxury upscale';
