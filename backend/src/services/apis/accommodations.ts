...
interface Accommodation {
  name: string;
  type: 'hotel' | 'hostel' | 'boutique' | 'apartment';
  priceRange: string;
  avgNight: string;
  rating: string;
  neighborhood: string;
...
const CITY_ACCOMMODATIONS: Record<string, Accommodation[]> = {
  'new york': [
...
function matchCity(city: string): Accommodation[] {
...
export const accommodationService = {
  async getAccommodations(city: string, budget?: string, type?: string): Promise<ToolResult<Accommodation[]>> {
    ...
    if (budget) {
      ...
      if (filtered.length > 0) accommodations = filtered;
    }
