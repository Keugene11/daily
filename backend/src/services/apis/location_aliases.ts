 ...
 */
export function resolveLocation(input: string, availableCities: string[], noAlias = false): string | null {
  const c = input.toLowerCase().trim();

  // Direct match
  if (availableCities.includes(c)) return c;

  // Partial match (input contains a city name or vice versa)
  for (const city of availableCities) {
    if (c.includes(city) || city.includes(c)) return city;
  }
  ...
  if (candidates) {
    for (const candidate of candidates) {
      if (availableCities.includes(candidate)) return candidate;
    }
  }
